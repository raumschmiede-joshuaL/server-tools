...
class TestBaseExceptionCommon(SavepointCase):
    ...
    def setUpClass(cls):
        ...
        from .purchase_test import (
            ExceptionRule,
            LineTestReverseParent,
            LineTestReverseSelf,
            PurchaseTest,
        )

        cls.loader.update_registry(
            (ExceptionRule, LineTestReverseParent, LineTestReverseSelf, PurchaseTest)
        )

        cls.partner = cls.env["res.partner"].create({"name": "Foo"})
        cls.po = cls.env["test.purchase"].create(
            {
        ...
        cls.sub_exception_rule_parent = cls.env["exception.rule"].create(
            {
                "name": "Amount less than 100",
                "description": "Line must have price greater than 100",
                "sequence": 9,
                "model": "test.purchase.line.reverse.parent",
                "code": "failed = self.amount < 100",
                "exception_type": "by_py_code",
            }
        )
        cls.sub_exception_rule_self = cls.env["exception.rule"].create(
            {
                "name": "Amount less than 100",
                "description": "Self line must have price greater than 100",
                "sequence": 9,
                "model": "test.purchase.line.reverse.self",
                "code": "failed = self.amount < 100",
                "exception_type": "by_py_code",
            }
        )
